...
// Version: 0.9.8 AI Core - Glassmorphism UI & VISITER Role Implemented
// Developer: I3AYY & AI Assistant (Medical Tech & CyberSec Expert)
// Update: First-time Setup Bypass, Caching Layer, Batch setValues, Least Privilege
// =========================================================================
...
// -------------------------------------------------------------------------
const DEFAULT_DB = {
  MAIN: 'xxx',        // Items, Stock_Balance
  UNIT: 'xxx',      // Units, ReagUnits, Analyzers, storageLocation, Company, ReagTypes
  USER: 'xxx',       // User
  CONFIG: 'xxx',  // Sticker_Config, App_Logo, Year_Config
  LOG: 'xxx',        // System_Logs
...
function checkDatabaseSetup() {
  ...
  if (!config.MAIN || config.MAIN.includes('ใส่_ID_ไฟล์') || config.MAIN === 'xxx' || 
      !config.USER || config.USER.includes('ใส่_ID_ไฟล์') || config.USER === 'xxx') {
  ...
  if (e.parameter.report) {
     return HtmlService.createHtmlOutput(`
       <div style="font-family: sans-serif; text-align: center; padding: 50px;">
         ...
         <p>กรุณากลับไปที่ Telegram หรือเข้าสู่ระบบ BioAgentics เพื่อดูข้อมูล</p>
       </div>
     `).setTitle('BioAgentics | Report Generator');
  }

  return HtmlService.createTemplateFromFile('index')
    .evaluate()
    .setTitle('BioAgentics | Lab Inventory System (v0.9.8 AI Core)')
...
function logSystem(action, detail, userId) {
  try {
    checkDatabaseSetup(); 
    const config = getDbConfig();
    if (config.LOG && !config.LOG.includes('ใส่_ID_ไฟล์') && config.LOG !== 'xxx') {
...
function sendTelegramNotification(unitName, messageText, replyMarkup = null, documentBlob = null, documentName = "", specificChatId = null) {
  try {
    const config = getDbConfig();
    if (!config.UNIT || config.UNIT.includes('ใส่_ID_ไฟล์') || config.UNIT === 'xxx') return;
    const ss = SpreadsheetApp.openById(config.UNIT);
    const sheet = ss.getSheetByName('Units');
    if (!sheet) return;
    const data = sheet.getDataRange().getValues();
...
function apiSetupTelegramWebhooks(userId) {
  try {
    ...
    if(!config.UNIT || config.UNIT.includes('ใส่_ID_ไฟล์') || config.UNIT === 'xxx') throw new Error("ไม่พบฐานข้อมูลหน่วยงาน");
    const ss = SpreadsheetApp.openById(config.UNIT);
    const sheet = ss.getSheetByName('Units');
    const data = sheet.getDataRange().getValues();
...
// -------------------------------------------------------------------------
// 4. AUTHENTICATION & SECURITY (With Setup Bypass)
// -------------------------------------------------------------------------
function verifyLogin(userId, password) {
  try {
    ...
    try {
      ...
      // หากเกิดข้อผิดพลาดในการเชื่อมต่อ หรือยังไม่ได้ตั้งค่า Google Sheet ID
      userSheetExists = false;
    }

    // ==========================================
    // 🔓 BYPASS MODE: สำหรับเข้าใช้งานเพื่อตั้งค่าระบบครั้งแรก
    // ==========================================
    if (!userSheetExists) {
      // ตรวจสอบข้อมูลล็อกอินแอดมินเริ่มต้น
      if (String(userId).toLowerCase() === 'admin' && String(password) === 'admin1234') {
        console.warn("Bypass Login Activated: Logged in using System Setup Admin account.");
        return { 
          success: true, 
          bypassed: true, 
          message: "เข้าสู่ระบบด้วยสิทธิ์ผู้ดูแลระบบเริ่มต้นสำเร็จ (กรุณาไปที่หน้าตั้งค่าฐานข้อมูลเพื่อใส่ Google Sheet ID ของคุณ)", 
          user: {
        ...
        return { 
          success: false, 
          message: "⚠️ ระบบฐานข้อมูลยังไม่ได้ตั้งค่า หรือไฟล์ ID ไม่ถูกต้องครับ\n\nกรุณาลงชื่อเข้าใช้ด้วย User: admin / Pass: admin1234 เพื่อเข้าไปทำการตั้งค่าฐานข้อมูลหลังบ้าน" 
        };
      }
    }

    // ==========================================
    // 🔐 NORMAL MODE: ตรวจสอบข้อมูลจาก Google Sheet จริง
    // ==========================================
    for (let i = 1; i < data.length; i++) {
...
function generateAndSendOTP(userId, email, name) {
  try {
    ...
    try {
      const config = getDbConfig();
      if(config.CONFIG && !config.CONFIG.includes('ใส่_ID_ไฟล์') && config.CONFIG !== 'xxx') {
    ...
    const htmlTemplate = `
        <div style="font-family: -apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif; color: #1e293b; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="text-align: center; margin-bottom: 24px;">
                ...
                <span style="font-size: 28px; font-weight: 700; color: #0ea5e9; vertical-align: middle; margin-left: 12px; letter-spacing: -0.5px; display: inline-block;">BioAgentics</span>
            </div>
            ...
            <div style="background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; padding: 24px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
                ...
                <p style="font-size: 13px; margin-bottom: 12px; color: #64748b;"><strong style="color: #ef4444;">ข้อควรระวัง (PDPA):</strong> โปรดอย่าแชร์รหัสนี้กับบุคคลอื่น ทีมงาน BioAgentics จะไม่ขอรหัสผ่านหรือ OTP ของคุณผ่านช่องทางใดๆ โดยเด็ดขาด</p>
            </div>
        </div>
    `;

    MailApp.sendEmail({ to: email, subject: "รหัส OTP สำหรับเข้าสู่ระบบ BioAgentics", htmlBody: htmlTemplate, name: "BioAgentics LIS" });
    return { success: true };
...
function verifyPasswordOnly(userId, password) {
  try {
    ...
    try {
      checkDatabaseSetup();
      SpreadsheetApp.openById(getDbConfig().USER);
      userSheetExists = true;
    } catch(e) {
      userSheetExists = false;
    }

    // Bypass ปลดล็อกหน้าจอสำหรับแอดมินทดลองระบบ
    if (!userSheetExists) {
      if (String(userId).toLowerCase() === 'admin' && String(password) === 'admin1234') {
        return { success: true };
      }
      return { success: false, message: 'รหัสผ่านเริ่มต้นไม่ถูกต้อง' };
    }
...
function getUserProfileById(userId) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][1]) === String(userId)) { 
      ...
      try {
        if(config.UNIT && !config.UNIT.includes('ใส่_ID_ไฟล์') && config.UNIT !== 'xxx') {
...
function getAvailableYears() {
  try {
    const config = getDbConfig(); 
    if(!config.CONFIG || config.CONFIG.includes('ใส่_ID_ไฟล์') || config.CONFIG === 'xxx') return [];
...
function apiCreateYearSheet(year, userId) {
  ...
  try {
    ...
    const config = getDbConfig();
    if(!config.CONFIG || config.CONFIG.includes('ใส่_ID_ไฟล์') || config.CONFIG === 'xxx') throw new Error("กรุณาระบุ Sheet ID สำหรับไฟล์ Config ก่อนสร้างปีงบประมาณ");
    const configSS = SpreadsheetApp.openById(config.CONFIG); 
...
function apiManualAddYearSheet(year, fileId, userId) {
  ...
  try {
    lock.waitLock(10000); checkDatabaseSetup();
    const config = getDbConfig();
    if(!config.CONFIG || config.CONFIG.includes('ใส่_ID_ไฟล์') || config.CONFIG === 'xxx') throw new Error("Missing Config DB ID");
    const configSS = SpreadsheetApp.openById(config.CONFIG); 
    let yearSheet = configSS.getSheetByName('Year_Config');
    if (!yearSheet) {
      yearSheet = configSS.insertSheet('Year_Config');
...
function apiGetSystemLogo() {
  try {
    const config = getDbConfig();
    if(!config.CONFIG || config.CONFIG.includes('ใส่_ID_ไฟล์') || config.CONFIG === 'xxx') return { success: false };
...
function apiSaveSystemLogo(base64Data, userId) {
  try {
    const config = getDbConfig();
    if(!config.CONFIG || config.CONFIG.includes('ใส่_ID_ไฟล์') || config.CONFIG === 'xxx') throw new Error("กรุณาตั้งค่า Config ID ในหน้าตั้งค่าฐานข้อมูลก่อน");
...
function apiGetStickerConfig() {
  try {
    ...
    if(!dbConfig.CONFIG || dbConfig.CONFIG.includes('ใส่_ID_ไฟล์') || dbConfig.CONFIG === 'xxx') {
...
function apiSaveStickerConfig(newConfig, userId) {
  ...
  try {
    const dbConfig = getDbConfig();
    if(!dbConfig.CONFIG || dbConfig.CONFIG.includes('ใส่_ID_ไฟล์') || dbConfig.CONFIG === 'xxx') throw new Error("กรุณาตั้งค่า Config ID ในหน้าตั้งค่าฐานข้อมูลก่อน");
...
function apiGetFormOptions() {
  try {
    const config = getDbConfig(); 
    if(!config.UNIT || config.UNIT.includes('ใส่_ID_ไฟล์') || config.UNIT === 'xxx') return { success: false, message: 'ไม่ได้ตั้งค่า Unit DB' };
...
function getItemsData(yearSheetId, dashboardFilterMonth) {
  try {
    ...
    let txReportMap = {};
    if (yearSheetId && config.CONFIG && !config.CONFIG.includes('ใส่_ID_ไฟล์') && config.CONFIG !== 'xxx') {
        try {
            const configSS = SpreadsheetApp.openById(config.CONFIG);
            let yearSheet = configSS.getSheetByName('Year_Config');
            if (yearSheet) {
...
function dailyLowStockAlert() {
  try {
    const config = getDbConfig();
    if (!config.MAIN || config.MAIN.includes('ใส่_ID_ไฟล์') || config.MAIN === 'xxx') return;
    const dbSS = SpreadsheetApp.openById(config.MAIN);
